...
def create_dataset(path):
    ...
    for i in range(1,10000):
        ...
        plt.savefig(path + '/lin_'+str(i)+'.png', bbox_inches='tight',pad_inches=0)
        plt.close() 
        fig = plt.plot(x,ft_geo)
        ax=plt.gca()
        ax.get_xaxis().set_visible(False)
        ax.get_yaxis().set_visible(False)
        plt.savefig(path + 'geo_'+str(i)+'.png', bbox_inches='tight',pad_inches=0)
        plt.close() 
        fig = plt.plot(x,ft_sin)
        ax=plt.gca()
        ax.get_xaxis().set_visible(False)
        ax.get_yaxis().set_visible(False)
        plt.savefig(path + '/sin_'+str(i)+'.png', bbox_inches='tight',pad_inches=0)
        plt.close() 
    print("Dataset Complete")

if __name__ == "create_dataset":
    create_dataset()
